...
**那个跟领域走的问题，不要你凭空选。** 不给 `--questions` 时它会去读命令，
并且把猜测和依据一起打出来：

```
$ whatitdid run session.jsonl
  看起来是 sre.v1（kubectl x20, get pods x5, namespace x1）
```

命令分不出来的时候它会直说并回落，而不是硬猜。跑完之后报告还会说这套问题集到底合不合适——
用错了世界的问题集，表现是步子大量掉进「归不了类」。同一条运维轨迹标三遍：
...
它抓的是「来自另一个世界」的问题集，不是「对你这个领域不够贴切」的那种——
`code.v1` 跟 `sre.v1` 共享四个选项，所以过了。注意把握几乎没动：
**标注器会一边很确定，一边把什么都塞进「归不了类」。**

### 给自己的领域写一套

```bash
whatitdid questions propose session.jsonl --out mine.json   # 从真实步子里起草
whatitdid questions check mine.json session.jsonl           # 先过检，再谈用
```

`check` 会在全程均匀取样的真实步子上标一遍，报四件事，每件都对着随包问题集在对口数据上的表现：

```
mine.v1，在 36 个真实步子上
  ok   答得出来      top choice median 0.93
  FAIL 选项都用得上   largest option 28%
  ok   跟着动作走     3% move with narration
  ok   问题不重复     0 correlated pairs
  还不能用：
    - never used: read_guide
```

没过检时退出码非零。上面那个草稿是真的：让模型给一次运行的动作命名，
它造出了一个从来没有步子落进去的类别——这正是这道检查存在的理由。
